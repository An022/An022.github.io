(() => {
  ...
  const levels = {
    ...
    2: {
      ...
      copy: 'Connect SOURCE to EXIT through DISCOVER → DESIGN → BUILD → DELIVER. Move the four reflectors by pushing them or dragging with the mouse; press Space or click to rotate.',
      player: 'tech',
      start: { x: 9, y: 5 },
      walls: [
        { x: 0, y: 0, label: '' },
      ...
      emitter: { x: 0, y: 4, direction: { x: 1, y: 0 } },
      receiver: { x: 9, y: 4 },
      targets: [
        { x: 1, y: 4, label: 'DISCOVER' },
        { x: 2, y: 2, label: 'DESIGN' },
        { x: 5, y: 1, label: 'BUILD' },
        { x: 8, y: 3, label: 'DELIVER' }
      ],
      mirrors: [
        { x: 3, y: 4, slash: false },
        { x: 3, y: 1, slash: true },
  ...
  function addMirror(mirror, index) {
    ...
    element.type = 'button';
    element.className = 'laser-mirror ' + (mirror.slash ? 'is-slash' : 'is-backslash') + (index === selectedMirror ? ' is-selected' : '');
    ...
    element.addEventListener('click', () => {
      ...
      rotateMirror(index, 'Reflector rotated. Follow the beam from the emitter.');
    });
    ...
    element.addEventListener('pointermove', (event) => {
      ...
      if (samePoint(next, mirror) || wallAt(next) || mirrorAt(next, index) || samePoint(next, players.tech) || laserDeviceAt(next)) return;
      mirror.x = next.x;
      mirror.y = next.y;
  ...
  function controlsFor(number) {
    ...
    if (number === 2) return '<span class="control-guide"><b>ARROWS</b> WALK + PUSH <b>SPACE</b> ROTATE IN FRONT <b>MOUSE</b> DRAG · CLICK TO ROTATE</span>';
  ...
  function startLevel(number) {
    ...
    undoMove.textContent = number === 2 ? 'Rotate nearby mirror · Space' : 'Undo move Z';
  ...
  function rotateNearbyMirror() {
    ...
    if (index < 0) {
      gameStatus.textContent = 'Move Bit beside a reflector, face it, then press Space. With a mouse, click any reflector.';
      return;
    }
