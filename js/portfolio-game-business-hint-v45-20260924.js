(() => {
  ...
  gameHint.setAttribute('role', 'status');
  gameHint.setAttribute('aria-live', 'assertive');
  gameHint.innerHTML = '<span>BUSINESS HINT</span><strong></strong><p></p>';
  arena.appendChild(gameHint);
  let hintTimer = 0;
  ...
  const levels = {
    1: {
      ...
      copy: 'Business judgment follows a sequence: define the real question, verify the evidence, test the downside, then make an accountable call. Push the crates onto any docks in that order.',
      player: 'business',
      start: { x: 4, y: 6 },
      techStart: { x: 10, y: 4 },
      storyPortal: { x: 0, y: 6 },
  ...
  function showBusinessHint(box) {
    const guidance = [
      ['Start with the question.', 'A sound business decision begins by defining what the business is actually trying to solve. Restore ASK WHY first.'],
      ['Evidence before judgment.', 'Validate the data before deciding what may go wrong. Restore READ DATA next.'],
      ['Test the downside.', 'The evidence is ready. Find the exception and understand its consequence before committing. Restore SPOT RISK next.'],
      ['Make the accountable call.', 'The question, evidence, and risk are clear. Restore MAKE CALL to finish the decision.']
    ][progress] || ['Follow the decision chain.', 'Define the question, verify the evidence, test the risk, then make the call.'];
    if (hintTimer) window.clearTimeout(hintTimer);
    gameHint.querySelector('strong').textContent = box.label + ' comes later — ' + guidance[0];
    gameHint.querySelector('p').textContent = guidance[1];
    gameHint.hidden = false;
    window.requestAnimationFrame(() => gameHint.classList.add('is-visible'));
    hintTimer = window.setTimeout(clearBusinessHint, 4600);
  }
  ...
  function transitionToFusion(message) {
    ...
    window.setTimeout(() => {
      ...
      walls = walls.filter((wall) =>
        !((wall.x === 2 || wall.x === 3) && wall.y === 0) &&
        !(returnPortal && samePoint(wall, returnPortal))
      );
      terrain.push({ x: 2, y: 0, kind: 'floor' });
      ...
      overlay.querySelector('strong').textContent = 'MOUNTAIN PASSAGE OPEN';
      overlay.classList.add('is-leaving');
      window.setTimeout(() => {
        overlay.remove();
