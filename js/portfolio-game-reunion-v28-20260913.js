(() => {
  ...
  function renderLaserMaze() {
    ...
    if (exitWall) exitWall.classList.toggle('is-broken', laserSolved);
  ...
  function transitionToFusion(message) {
    ...
    window.setTimeout(() => {
      const overlay = document.createElement('div');
      overlay.className = 'reunion-transition';
      overlay.setAttribute('role', 'status');
      overlay.innerHTML = '<span>COMPANION RELEASED</span><div class="reunion-pair" aria-hidden="true"><i class="is-green"></i><b>✦</b><i class="is-blue"></i></div><strong>TOGETHER TO THE ICE RELAY →</strong>';
      play.appendChild(overlay);
      window.setTimeout(() => {
        startLevel(3);
        transitioning = true;
  ...
  function evaluateLaserMaze(message) {
    ...
    renderProgress();
    if (laserSolved) {
      transitionToFusion('The east wall dissolves. A second signal answers from the decision room.');
  ...
  function moveTechInLaser(key) {
    ...
    renderArena();
    gameStatus.textContent = 'Walk into a reflector to push it, or press Space while facing one.';
  }
