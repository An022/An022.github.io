(() => {
  ...
  function addLabDoor() {
    ...
    if (!exit) return;
    const element = document.createElement('div');
    element.className = 'lab-door' + (roomExitOpen ? ' is-open' : '');
    element.setAttribute('role', 'img');
    element.setAttribute('aria-label', roomExitOpen ? 'Open exit door' : 'Locked exit door');
  ...
  function moveChapterOneTech(key) {
    ...
    if (!roomKeyCollected && samePoint(next, currentLevel().key)) {
      ...
      roomExitOpen = true;
      renderArena();
      gameStatus.textContent = 'Key collected. The yellow door clicks open.';
      return;
    }
    if (samePoint(next, exit) && roomExitOpen) {
      renderArena();
      finishRoom('Bit unlocks the east door and slips into the signal lab.');
      return;
    }
    renderArena();
