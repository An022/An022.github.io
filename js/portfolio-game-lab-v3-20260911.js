(() => {
  ...
  const levels = {
    ...
    2: {
      ...
      copy: 'Guide Bit through the signal lab. Walk into a reflector to push it, face one and press Space to rotate it, or use the mouse to drag and click. Route the beam through the relays in order.',
      player: 'tech',
      start: { x: 9, y: 5 },
      walls: [
        { x: 0, y: 0, label: '' },
  ...
  function moveTechInLaser(key) {
    ...
    if (mirrorIndex >= 0) {
      ...
      if (!inside(destination) || wallAt(destination) || mirrorAt(destination, mirrorIndex) || samePoint(destination, players.tech)) {
