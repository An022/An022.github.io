(() => {
  ...
  function startLevel(number) {
    ...
    if (number === 1) {
      ...
      gameStatus.textContent = 'Bit enters from the west. Blue beacons are solid; turn them red in sequence.';
    } else {
  ...
  function transitionToLevel(number, message) {
    ...
    overlay.innerHTML = '<span>ACCESS GRANTED</span><strong>FOLLOWING BIT →</strong>';
  ...
  function beginChapterOneExit() {
    ...
    renderArena();
    gameStatus.textContent = 'The Why Lens is restored. A key drops inside Bit’s brick room.';
  }
  ...
  function moveChapterOneTech(key) {
    ...
    if (!inside(next) || wallAt(next) || !walkable(next)) {
      gameStatus.textContent = 'The brick room keeps Bit on this tiny path.';
      return;
    }
    ...
    if (samePoint(next, exit) && roomExitOpen) {
      renderArena();
      transitionToLevel(2, 'Bit slips through the open passage.');
      return;
    }
    renderArena();
