(() => {
  ...
  const levels = {
    1: {
      ...
      boxes: [
        { x: 4, y: 8, label: 'ASK WHY', order: 0, target: { x: 3, y: 8 } },
        { x: 8, y: 4, label: 'READ DATA', order: 1, target: { x: 7, y: 4 } },
        { x: 3, y: 3, label: 'SPOT RISK', order: 2, target: { x: 5, y: 3 } },
        { x: 4, y: 4, label: 'MAKE CALL', order: 3, target: { x: 2, y: 7 } }
      ]
    },
    2: {
      mode: 'lasermaze',
  ...
  function snapshotLaserMaze() {
    return {
      player: { ...players.tech },
      facing: { ...techFacing },
      mirrors: mirrors.map((mirror) => ({ x: mirror.x, y: mirror.y, slash: mirror.slash })),
      selectedMirror
    };
  }
