(() => {
  ...
  function addLabBreach() {
    ...
    [currentLevel().receiver, currentLevel().returnExit].forEach((point) => {
  ...
  function renderArena() {
    ...
    walls.forEach((wall) => {
      const openLabWall = stage === 2 && labExitReady && wall.x === cols - 1 &&
        (wall.y === currentLevel().receiver.y || wall.y === currentLevel().returnExit.y);
  ...
  function wallAt(point) {
    if (stage === 2 && labExitReady && point.x === cols - 1 && currentLevel().returnExit &&
      (point.y === currentLevel().receiver.y || point.y === currentLevel().returnExit.y)) return false;
  ...
  function transitionToFusion(message) {
    ...
    window.setTimeout(() => {
      ...
      roomExitOpen = true;
      walls = walls.filter((wall) => !(wall.x === 3 && wall.y === 0));
      ...
      overlay.querySelector('strong').textContent = 'A CAVE OPENS ABOVE';
      overlay.classList.add('is-leaving');
      window.setTimeout(() => {
        overlay.remove();
        transitioning = false;
  ...
  function moveReturnPlayer(type, key) {
    ...
    gameStatus.textContent = 'Guide both travelers to the cave in the top row, fourth column.';
  }
  ...
  function tryPushBox(type, index, delta) {
    ...
    if (reachedDock && box.order === progress) {
      box.locked = true;
      progress += 1;
  ...
  function evaluateLaserMaze(message) {
    ...
    if (laserSolved) {
      ...
      undoMove.disabled = true;
      renderArena();
      gameStatus.textContent = 'The east wall is gone. Keep reflectors in place and walk around to the lower-right opening.';
  ...
  function rotateNearbyMirror() {
    ...
    if (labExitReady) {
      gameStatus.textContent = 'The route is complete. Reflectors stay fixed; walk around to the lower-right opening.';
      return;
    }
