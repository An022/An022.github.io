(() => {
  ...
  const levels = {
    ...
    2: {
      ...
      mirrors: [
        { x: 1, y: 5, label: 'SYSTEMS', slash: false },
        { x: 3, y: 1, label: 'APIs', slash: true },
        { x: 5, y: 0, label: 'DATA', slash: true },
        { x: 6, y: 5, label: 'DEBUG', slash: true },
        { x: 9, y: 3, label: 'UX', slash: true },
        { x: 7, y: 0, label: 'SHIP', slash: false }
      ]
    },
    3: {
      mode: 'switchboard',
  ...
  function addMirror(mirror, index) {
    ...
    element.setAttribute('aria-label', mirror.label + ' reflector. ' + (mirror.slash ? 'Slash' : 'Backslash') + ' orientation.');
    element.innerHTML = '<span aria-hidden="true"></span><small></small>';
    element.querySelector('span').textContent = mirror.slash ? '/' : '\\';
    element.querySelector('small').textContent = mirror.label;
    ...
    element.addEventListener('click', () => {
      ...
      if (selectedMirror === index) rotateSelectedMirror();
        ...
        gameStatus.textContent = mirror.label + ' selected. Arrows move; Space rotates.';
        renderArena();
      }
    });
    arena.appendChild(element);
  ...
  function traceLaser() {
    ...
    for (let step = 0; step < 120; step += 1) {
      ...
      laserTargets.forEach((target) => {
        if (samePoint(target, point)) target.hit = true;
      });
  ...
  function startLevel(number) {
    ...
    if (number === 1) {
      ...
      gameStatus.textContent = 'SYSTEMS selected. Arrows move it, Space rotates it, and Tab selects the next skill.';
    } else {
  ...
  function evaluateLaserMaze(message) {
    ...
    renderProgress();
    if (laserSolved) {
      finishRoom('All four relays are live. Bit turned six technical skills into one working signal.');
    } else {
  ...
  function moveSelectedMirror(key) {
    ...
    if (!inside(destination) || wallAt(destination) || mirrorAt(destination, selectedMirror) || samePoint(destination, players.tech)) {
      gameStatus.textContent = mirror.label + ' has no room to move there.';
      bump('[data-mirror="' + String(selectedMirror) + '"]');
      return;
    }
    mirror.x = destination.x;
    mirror.y = destination.y;
    evaluateLaserMaze(mirror.label + ' moved. The beam recalculated instantly.');
  }
  ...
  function rotateSelectedMirror() {
    ...
    evaluateLaserMaze(mirror.label + ' rotated. Watch where the beam changes direction.');
  }
