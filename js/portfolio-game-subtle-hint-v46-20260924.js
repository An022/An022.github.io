(() => {
  ...
  const levels = {
    1: {
      ...
      copy: 'Business judgment follows a sequence. Restore the room as you would handle an everyday decision: understand the situation, check the facts, consider the consequences, and only then act.',
      player: 'business',
      start: { x: 4, y: 6 },
      techStart: { x: 10, y: 4 },
      storyPortal: { x: 0, y: 6 },
  ...
  function showBusinessHint() {
    const guidance = [
      ['Something must come first.', 'Before facts, risks, or action can help, what does the decision maker need to understand?'],
      ['What would make the judgment reliable?', 'The situation is understood. Think about what you would check before drawing a conclusion.'],
      ['Pause before acting.', 'You understand the situation and have supporting facts. What should you consider before committing?'],
      ['Turn the thinking into action.', 'The earlier steps have prepared the decision. What kind of step belongs at the end?']
    ][progress] || ['Reconsider the chain.', 'Think about what needs to happen before this step can be trusted.'];
    if (hintTimer) window.clearTimeout(hintTimer);
    gameHint.querySelector('strong').textContent = 'This step came too soon. ' + guidance[0];
