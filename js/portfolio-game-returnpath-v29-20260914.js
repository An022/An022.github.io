(() => {
  ...
  function clearArena() {
    arena.querySelectorAll('.terrain-tile, .game-crate, .workshop-wall, .goal-socket, .scene-hazard, .moon-switch, .moon-gate, .meeting-signal, .laser-mirror, .laser-target, .laser-emitter, .laser-receiver, .laser-path, .lab-key, .lab-door, .story-portal, .ice-passage-tile').forEach((element) => element.remove());
  }
  ...
  function addBox(box, index) {
    ...
    element.className = 'game-crate is-' + (box.owner || currentLevel().player) + mirrorClass + (onGoal ? ' is-on-goal is-locked' : '');
  ...
  function renderLaserMaze() {
    ...
    mirrors.forEach((mirror, index) => {
      if (!labExitReady || !samePoint(mirror, players.tech)) addMirror(mirror, index);
    });
  ...
  function transitionToFusion(message) {
    ...
    window.setTimeout(() => {
      ...
      players.tech = { x: 9, y: 6 };
      roomExitOpen = true;
      levelLabel.textContent = 'INTERLUDE / THE RETURN PATH';
      levelTitle.textContent = 'Reunites the travelers.';
      stageCounter.textContent = 'HOMEBOUND / 03';
      activeControls.innerHTML = '<span class="control-guide"><b>AUTOPILOT</b> FOLLOW THE ICE PATH</span>';
      gameStatus.textContent = 'The blue traveler returns. Together, they follow the new frozen passage.';
      document.querySelector('.business-pad').classList.add('is-inactive');
      document.querySelector('.tech-pad').classList.add('is-inactive');
      position(businessPlayer, players.business);
      position(techPlayer, players.tech);
      renderArena();
      renderProgress();
      arena.classList.add('is-return-cinematic');

      for (let x = 0; x <= 3; x += 1) {
        const tile = document.createElement('div');
        tile.className = 'ice-passage-tile';
        tile.style.setProperty('--ice-step', String(3 - x));
        position(tile, { x, y: 6 });
        arena.appendChild(tile);
      }

      overlay.querySelector('span').textContent = 'COMPANION FOUND';
      overlay.querySelector('strong').textContent = 'THE ICE ROAD ANSWERS';
      overlay.classList.add('is-leaving');
      ...
      const arrive = () => {
        arrived += 1;
        if (arrived < 2) return;
        gameStatus.textContent = 'Two signals enter the ice road. One ship is waiting.';
        window.setTimeout(() => {
          startLevel(3);
  ...
  function evaluateLaserMaze(message) {
    ...
    if (laserSolved) {
      labExitReady = true;
      undoMove.disabled = true;
      gameStatus.textContent = 'The east wall is gone. Walk the blue traveler through the opening on the right.';
  ...
  function rotateNearbyMirror() {
    ...
    if (labExitReady) {
      gameStatus.textContent = 'The route is complete. Walk through the opening on the right.';
      return;
    }
  ...
  function moveTechInLaser(key) {
    ...
    const exitWall = walls.find((wall) => wall.kind === 'exit');
    if (labExitReady && exitWall && samePoint(next, exitWall)) {
      players.tech = next;
      position(techPlayer, players.tech);
    ...
    if (mirrorIndex >= 0) {
      if (labExitReady) {
        ...
        techPlayer.classList.toggle('is-stepping');
        renderArena();
        gameStatus.textContent = 'The aligned reflector lets the traveler pass. One more step reaches the breach.';
        return;
      }
      ...
      players.tech = next;
      position(techPlayer, players.tech);
      evaluateLaserMaze('Reflector pushed one tile. Face it and press Space to rotate.');
      return;
    }
    moveHistory.push(snapshotLaserMaze());
    undoMove.disabled = false;
